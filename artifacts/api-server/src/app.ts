...
// Phase 6.8 — pattern list moved to src/lib/auth-bypass-patterns.ts so
// the webhook-audit smoke can import it without triggering app.ts's
// boot side-effects (Sentry init, DB pools, express.listen). Any new
// bypass entry lives THERE, not here.
...
// Phase 6.8 — startup assertion: every webhook-shaped route registered
// on the apiRouter MUST have a matching entry in AUTH_BYPASS_PATTERNS
// or gatewayAuth will 401 the request before the handler's internal
// signature check even runs. Phase 6.4 shipped two callbacks without
// entries and lost six voicemails on 2026-08-15 before we caught it.
// Fails LOUD at boot rather than in production. See src/lib/webhook-audit.ts.
{
  const { enumerateRoutePaths, findMissingBypassEntries } =
    require("./lib/webhook-audit") as typeof import("./lib/webhook-audit");
  const registered = enumerateRoutePaths(router as unknown as { stack?: unknown[] }, "/api");
  const missing = findMissingBypassEntries(registered, AUTH_BYPASS_PATTERNS);
  if (missing.length > 0) {
    const msg =
      "Startup audit FAILED — the following webhook-shape route(s) are registered but MISSING from AUTH_BYPASS_PATTERNS (src/lib/auth-bypass-patterns.ts). gatewayAuth will 401 them before the handler runs, silently losing data:\n" +
      missing.map((p) => "  " + p).join("\n") +
      "\nAdd a bypass entry for each and redeploy.";
    console.error(msg);
    throw new Error(msg);
  }
  console.log(
    `[webhook-audit] ${registered.length} route(s) enumerated; every webhook-shape route has a bypass entry.`,
  );
}
